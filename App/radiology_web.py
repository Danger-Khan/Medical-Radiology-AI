#!/usr/bin/env python3
"""
Medical Radiology AI — Streamlit (responsive web) edition
=============================================================
A responsive web UI, sibling to the Tkinter desktop console (radiology_console.py) — same shared
logic, same model backend (inference.py: Roboflow-hosted models, the offline_cv.py pixel-diff
heuristic, a locally-trained classifier, offline HF models, and the SIMULATED scenario picker,
tried in whichever order is measurably best per modality — see Documentations/MODEL_SOURCES.md),
same SQLite cache. All shared logic (constants, imaging, simulated scenarios, DB, reports, the
run_triage() dispatcher, draw_bbox()) is imported straight from radiology_console.py rather than
duplicated — its Tkinter code never runs unless that module is executed directly, so importing it
here is safe.

Run with:  streamlit run radiology_web.py
"""
import time
...
import radiology_console as core  # noqa: the shared logic module — see module docstring above

st.set_page_config(page_title="Medical Radiology AI", page_icon="🩻", layout="wide", initial_sidebar_state="expanded")

# ============================================================
# RESPONSIVE LIGHT BLUE / LIGHT GRAY THEME (same color tokens as radiology_console.py's `C`)
# ============================================================
C = core.C
CSS = f"""
<style>
.stApp {{ background: {C['bg']}; color: {C['text']}; }}
.block-container {{ padding-top: 2.2rem !important; padding-bottom: 2.2rem !important; max-width: 1300px !important; }}
section[data-testid="stSidebar"] {{ background: {C['surface']} !important; }}
h1, h2, h3, h4 {{ color: {C['text']} !important; }}
/* Menu on the right: flip the app's outer flex row so the sidebar (the container's first child)
   renders after the main content instead of before it. Depends on Streamlit's current DOM
   structure -- a documented, widely-used hack, not an official API, so it may need revisiting on
   a future Streamlit upgrade. */
div[data-testid="stAppViewContainer"] {{ flex-direction: row-reverse !important; }}
section[data-testid="stSidebar"] {{ border-left: 1px solid {C['border']}; border-right: none !important; }}
...
.badge {{ display: inline-flex; padding: 6px 14px; border-radius: 24px; font-size: 0.72rem; font-weight: 600; }}
...
.card {{ background: {C['surface']}; border: 1px solid {C['border']}; border-radius: 18px; padding: 20px; margin: 10px 0;
  box-shadow: 0 1px 3px rgba(30,41,59,0.06); }}
.metric-tile {{ background: {C['surface']}; border: 1px solid {C['border']}; border-radius: 14px; padding: 16px; text-align: center; }}
...
.verdict-box {{ border-radius: 18px; padding: 24px; text-align: center; margin: 10px 0; }}
...
.console-log {{ background: {C['console_bg']}; color: {C['console_fg']}; border: 1px solid {C['border_light']}; border-radius: 14px;
  padding: 16px; font-family: 'Consolas', monospace; font-size: 0.76rem; line-height: 1.8; max-height: 260px; overflow-y: auto; }}
.alert-card {{ border-radius: 14px; padding: 16px; margin: 8px 0; border-left: 4px solid; }}
...
def init_state():
    ...
    defaults = {
        ...
        "cache_saved": False, "clinician_notes": "",
    }
...
init_state()
core._urdu["on"] = st.session_state.urdu_mode  # sync radiology_console's t() to this session's language choice
t = core.t
...
def render_sidebar():
    with st.sidebar:
        st.markdown(
            ...
            'padding:14px;margin-bottom:14px;color:#fff;"><b>🩻 Medical Radiology AI</b>'
        ...
        st.session_state.clinician_notes = st.text_area(
            "Clinician Notes", value=st.session_state.get("clinician_notes", ""), height=90,
            label_visibility="collapsed", placeholder="Included in the report if not empty.")
        ...
        with st.expander("ℹ️ About"):
            st.caption(
                "Medical Radiology AI — offline-first triage console for Mammography, "
                "Tuberculosis, and Bone X-Ray imaging. See "
                "Documentations/MODEL_SOURCES.md for what backs each result.")
...
def save_cache_action(selected_model):
    ...
    data = {
        ...
        "clinician_notes": st.session_state.clinician_notes,
    }
...
def render_dashboard(selected_model):
    st.markdown(f"""<div class="page-header"><h1>🩻 Medical Radiology AI</h1>
    ...
    with col_img:
        ...
        if st.session_state.inference_done and st.session_state.current_result:
            ...
            m2.markdown(f'<div class="metric-tile"><div class="label">{t("Confidence")}</div><div class="value" style="color:{C["primary"]};">{r["confidence"]:.1f}%</div></div>', unsafe_allow_html=True)
    ...
    with col_meta:
        ...
        if st.session_state.inference_done and st.session_state.current_result:
            ...
            elif "Bone" in selected_model:
                opts1, opts2, l1, l2 = core.BONE_STATUS_OPTIONS, core.BONE_TYPE_OPTIONS, "Bone Status", "Fracture Type"
            else:
...
st.markdown(
    ...
    f'Medical Radiology AI (Streamlit) — same real models as the desktop console, browser UI</div>',
    unsafe_allow_html=True)
